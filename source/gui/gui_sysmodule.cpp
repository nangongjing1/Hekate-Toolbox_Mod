...
GuiSysmodule::GuiSysmodule() : Gui() {
    ...
    for (auto &sysmodule : this->m_sysmodules) {
        ...
        sysmoduleButton->drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
            ...
            gui->drawTextAligned(font20, x + 420, y + 50, this->m_runningSysmodules.find(sysmodule.first) != this->m_runningSysmodules.end() ? currTheme.selectedColor : Gui::makeColor(0xB8, 0xBB, 0xC2, 0xFF), this->m_runningSysmodules.find(sysmodule.first) != this->m_runningSysmodules.end() ? "开启" : "关闭", ALIGNED_LEFT);
        };
            ...
            if (kdown & HidNpadButton_A) {
                ...
                if (this->m_runningSysmodules.find(sysmodule.first) != this->m_runningSysmodules.end()) {
                    if (!sysmodule.second.requiresReboot) {
                        ...
                        (new MessageBox("修改此后台服务需要重启生效。\n 请重启主机以应用修改。", MessageBox::OKAY))->show();
                    }
...
void GuiSysmodule::draw() {
    ...
    if (anyModulesPresent)
        Gui::drawTextAligned(font20, Gui::g_framebuffer_width / 2, 150, currTheme.textColor, "选择要运行的后台服务(Sysmodules)。\n 由于内存限制，可能无法同时启动所有服务。", ALIGNED_CENTER);
    else
        Gui::drawTextAligned(font20, Gui::g_framebuffer_width / 2, 550, currTheme.textColor, "您目前未安装任何受支持的后台服务。\n 如需使用此功能，请安装受支持的后台服务(NSP格式)。", ALIGNED_CENTER);
